...
| Multi-agent workflows | Partially implemented (gaps) | A real run-scoped multi-agent runtime now exists with Planner, Evidence Normalizer, Risk Reviewer, and Reporter, parallel execution where possible, handoff tracking, and step outputs in Run Detail | Still limited to current-run memory only, no retries/tool sandboxing, and no deeper cross-run memory or autonomous tool execution yet |
